...
Horsepower/Darkplaces project. This is copied from the [Coreboot coding style](https://doc.coreboot.org/coding_style.html),
which itself was copied from the [Linux kernel coding style](http://git.kernel.org/cgit/linux/kernel/git/torvalds/linux.git/plain/Documentation/CodingStyle?id=HEAD)

Sorry, was too lazy to convert .rst to Markdown.

Also, much of this document is a **direct copy and paste** of the above source.
Some sections (obviously) will not apply as this is a game engine rather than
an operating system kernel (or firmware), but I haven't found the time to go 
through all of this. I did make one or two name changes and removed the whole 
section on Kconfig as we don't use that build system, but for the rest, use 
common sense until I get around to this.

Also, we use the zone allocator. Do not use malloc directly even if it tells
you to. Please and thank you.
...
IfÂ theÂ nameÂ ofÂ aÂ functionÂ isÂ anÂ actionÂ orÂ anÂ imperativeÂ command,
theÂ functionÂ shouldÂ returnÂ anÂ error-codeÂ integer.Â Â IfÂ theÂ name
isÂ aÂ predicate,Â theÂ functionÂ shouldÂ returnÂ aÂ "succeeded"Â boolean.
