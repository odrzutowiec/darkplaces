...
cvar_t sv_worldbasename = {CVAR_READONLY, "sv_worldbasename", "", "name of current worldmodel without maps/ prefix or extension", vstring};

cvar_t sv_disablenotify = {0, "sv_disablenotify", "0", "suppress broadcast prints when certain cvars are changed (CVAR_NOTIFY flag in engine code)", vboolean};
cvar_t coop = {0, "coop","0", "coop mode, 0 = no coop, 1 = coop mode, multiple players playing through the singleplayer game (coop mode also shuts off deathmatch)", vboolean};
cvar_t deathmatch = {0, "deathmatch","0", "deathmatch mode, values depend on mod but typically 0 = no deathmatch, 1 = normal deathmatch with respawning weapons, 2 = weapons stay (players can only pick up new weapons)", vinteger, "0", "2"};
cvar_t fraglimit = {CVAR_NOTIFY, "fraglimit","0", "ends level if this many frags is reached by any player", vinteger, "0"};
cvar_t gamecfg = {0, "gamecfg", "0", "unused cvar in quake, can be used by mods", vboolean};
cvar_t noexit = {CVAR_NOTIFY, "noexit","0", "kills anyone attempting to use an exit", vboolean};
cvar_t nomonsters = {0, "nomonsters", "0", "unused cvar in quake, can be used by mods", vboolean};
cvar_t pausable = {0, "pausable","1", "allow players to pause or not (otherwise, only the server admin can)", vboolean};
...
// check for changes to be sent over the reliable streams
	for (i = 0, host_client = svs.clients;i < svs.maxclients;i++, host_client++)
	{
		...
		if(sv_gameplayfix_nexuiz_evilfrags.integer)
